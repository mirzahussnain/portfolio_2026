  ...
  const onSubmit = (data: any) => {
    ...
    if (data.email === 'admin@example.com' && data.password === 'admin123') {
      dispatch(setCredentials({
        user: { name: 'Super User', email: data.email },
        token: '12345678abc',
        ...
        <div className="glass-card w-full rounded-2xl border border-glass-border p-8 sm:p-10 shadow-2xl">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5 w-full">
            <div className="space-y-2">
              ...
              {errors.email && <span className="text-xs text-red-400">Email required</span>}
            </div>

            <div className="space-y-2">
              <div className="flex justify-between items-center ml-1">
                <label className="text-sm font-medium text-slate-300">Password</label>
                <button type="button" className="text-xs font-medium text-primary hover:text-primary/80 transition-colors">Forgot password?</button>
              </div>
              <div className="relative flex items-center w-full rounded-lg bg-white/5 border border-white/10 transition-colors focus-within:border-primary">
              ...
              {errors.password && <span className="text-xs text-red-400">Password required</span>}
            </div>

            <button 
              type="submit"
              className="mt-2 w-full h-12 rounded-lg bg-primary hover:bg-[#0aa6da] text-background-dark font-bold text-sm transition-all transform active:scale-[0.98] shadow-lg shadow-primary/30 flex items-center justify-center gap-2 cursor-pointer"
            >
              <span>Sign In</span>
